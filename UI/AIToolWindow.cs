...
namespace DevTools.Humankind.GUITools.UI
{
    ...
    {
      if (Amplitude.Mercury.AI.Preferences.ControlledByHumanBindingAfterStartup == ControlledByHumanBinding.DeactivateAIOnAllEmpires || Amplitude.Mercury.AI.Preferences.ExecuteAIOnMainThread)
      {
    ...
    private void OnDrawAIWindow()
    {
      ...
      this.toggleAiState = GUILayout.Toggle(this.toggleAiState, "AI State", (GUIStyle) "PopupWindow.Button");
      GUILayout.FlexibleSpace();
      if (this.toggleAiState)
        Amplitude.Mercury.AI.Preferences.ControlledByHumanBindingAfterStartup = (ControlledByHumanBinding) GUILayout.SelectionGrid((int) Amplitude.Mercury.AI.Preferences.ControlledByHumanBindingAfterStartup, this.controlledByHumanBindingNames, 1, (GUIStyle) "PopupWindow.ToolbarButton");
      GUILayout.EndHorizontal();
      Amplitude.Mercury.AI.Preferences.PreventAttackMissions = GUILayout.Toggle(Amplitude.Mercury.AI.Preferences.PreventAttackMissions, "Prevent Attack Missions", (GUIStyle) "PopupWindow.Button");
      GUILayout.BeginHorizontal();
      GUILayout.Label("Debug");
      GUILayout.FlexibleSpace();
      if (GUILayout.Button("Force decision", (GUIStyle) "PopupWindow.Button"))
      {
        string address = "http://localhost:1477/api/ai-controller";
        string data = "{\"ReloadAIAssembly\" : false , \"PauseAIDecisions\" : false, \"ForceNewAIDecision\" : true }";
        using (WebClient webClient = new WebClient())
          webClient.UploadString(address, data);
      }
      if (GUILayout.Button("Open inspector", (GUIStyle) "PopupWindow.Button"))
        UnityEngine.Application.OpenURL("http://localhost:1477/");
      GUILayout.EndHorizontal();
      GUILayout.BeginHorizontal();
      GUILayout.Label("Overlay");
      GUILayout.FlexibleSpace();
      int num = GUILayout.SelectionGrid((int) DebugControl.WantedDebugger, this.debuggerTypeNames, 4, (GUIStyle) "PopupWindow.ToolbarButton");
    ...
    private void OnDrawDiplomacyDebugger()
    {
      ...
      GUILayout.FlexibleSpace();
      int num1 = GUILayout.SelectionGrid(DebugControl.DiplomaticAnalysisEmpireIndex, this.empireNames, this.empireNames.Length, (GUIStyle) "PopupWindow.ToolbarButton");
      if (num1 != DebugControl.DiplomaticAnalysisEmpireIndex || (double) Time.realtimeSinceStartup > (double) this.nextRefresh)
      {
        if (num1 != DebugControl.DiplomaticAnalysisEmpireIndex)
          DebugControl.TerritoryRequestResult = (JObject) null;
        HttpWebRequest httpWebRequest = (HttpWebRequest) WebRequest.Create("http://localhost:1477/api/empires/" + num1.ToString() + "/brain/analysis-data/all-diplomatic-relations/");
      ...
      GUILayout.FlexibleSpace();
      int num2 = GUILayout.SelectionGrid((int) DebugControl.WantedDiplomaticAnalaysis, this.diplomaticAnalysisTypeNames, 3, (GUIStyle) "PopupWindow.ToolbarButton");
      ...
      if (!DebugControl.CursorPosition.IsWorldPositionValid())
        return;
      int territoryIndex = (int) Amplitude.Mercury.Interop.AI.Snapshots.World.TileInfo[DebugControl.CursorPosition.ToTileIndex()].TerritoryIndex;
      Amplitude.Mercury.Interop.AI.Entities.Territory territory = Amplitude.Mercury.Interop.AI.Snapshots.World.Territories[territoryIndex];
      if (territory.Settlement == null)
        return;
      int empireIndex = territory.Settlement.Empire.EmpireIndex;
      if (empireIndex >= Amplitude.Mercury.Interop.AI.Snapshots.Game.MajorEmpires.Length || empireIndex == DebugControl.DiplomaticAnalysisEmpireIndex)
        return;
      JToken jtoken = DebugControl.DiplomacyRequestResult["Data"][(object) empireIndex];
      GUILayout.Label("Attitude : " + System.Enum.GetName(typeof (AIBehaviourState), (object) jtoken[(object) "DiplomaticContext"][(object) "StateMachineContext"][(object) "CurrentState"][(object) "Id"].ToObject<AIBehaviourState>()));
      GUILayout.Label(string.Format("Rapport: {0}", (object) jtoken[(object) "RapportScore"][(object) "Value"]));
      GUILayout.Label(string.Format("Superiority: {0}", (object) jtoken[(object) "SuperiorityScore"][(object) "Value"]));
    }
    ...
    private void OnDiplomacyRequestEnd(IAsyncResult asynchronousResult)
    {
      try
      {
        using (HttpWebResponse response = (HttpWebResponse) ((WebRequest) asynchronousResult.AsyncState).EndGetResponse(asynchronousResult))
        {
    ...
    private void OnDrawTerritoryDebugger()
    {
      ...
      GUILayout.Label("Empire");
      GUILayout.FlexibleSpace();
      int wantedEmpire = GUILayout.SelectionGrid(DebugControl.TerritoryAnalysisEmpireIndex, this.empireNames, 8, (GUIStyle) "PopupWindow.ToolbarButton");
      if (wantedEmpire != DebugControl.TerritoryAnalysisEmpireIndex || (double) Time.realtimeSinceStartup > (double) this.nextRefresh)
      {
      ...
      GUILayout.Label("Analysis");
      GUILayout.FlexibleSpace();
      int num = GUILayout.SelectionGrid((int) DebugControl.WantedTerritoryAnalysis, this.territoryAnalysisTypeNames, 3, (GUIStyle) "PopupWindow.ToolbarButton");
      ...
      if (!DebugControl.CursorPosition.IsWorldPositionValid())
        return;
      int territoryIndex = (int) Amplitude.Mercury.Interop.AI.Snapshots.World.TileInfo[DebugControl.CursorPosition.ToTileIndex()].TerritoryIndex;
      GUILayout.Label(string.Format("Position {0} | Territory {1}", (object) DebugControl.CursorPosition, (object) territoryIndex));
      try
      {
        ...
        switch (DebugControl.WantedTerritoryAnalysis)
        {
          ...
          case DebugControl.TerritoryAnalysisType.Threat:
            GUILayout.Label("Max ground enemy strength :" + jtoken[(object) "MaxGroundEnemyStrength"][(object) "Value"]?.ToString());
            GUILayout.Label("Max naval enemy strength :" + jtoken[(object) "MaxNavalEnemyStrength"][(object) "Value"]?.ToString());
          ...
          case DebugControl.TerritoryAnalysisType.WantedGroundArmies:
            GUILayout.Label("WantedGroundArmies: " + jtoken[(object) "WantedGroundArmies"].ToObject<HeuristicFloat>().ToString());
            break;
          case DebugControl.TerritoryAnalysisType.WantedNavalArmies:
            GUILayout.Label("WantedNavalArmies: " + jtoken[(object) "WantedNavalArmies"].ToObject<HeuristicFloat>().ToString());
            break;
          case DebugControl.TerritoryAnalysisType.Strategic:
            GUILayout.Label("Strategic: " + jtoken[(object) "StrategicScore"].ToObject<HeuristicFloat>().ToString());
            break;
          case DebugControl.TerritoryAnalysisType.Exploration:
            GUILayout.Label("ExplorationScore :" + jtoken[(object) "ExplorationScore"][(object) "Value"]?.ToString());
            GUILayout.Label("Distance from settlement :" + jtoken[(object) "DistanceFromSettlements"]?.ToString());
            for (int index = 0; index < jtoken[(object) "TerritoryTags"].Count<JToken>(); ++index)
              GUILayout.Label(jtoken[(object) "TerritoryTags"][(object) index][(object) "PrettyName"].ToString());
            break;
          case DebugControl.TerritoryAnalysisType.PathFind:
            ...
            DebugControl.IncludeCloseBorder = GUILayout.Toggle(DebugControl.IncludeCloseBorder, "IncludeCloseBorder");
            break;
        }
      }
      catch (Exception ex)
    ...
    private void RefreshTerritoryRequestIfNeeded(int wantedEmpire)
    {
      if (wantedEmpire != DebugControl.TerritoryAnalysisEmpireIndex)
        DebugControl.TerritoryRequestResult = (JObject) null;
      HttpWebRequest httpWebRequest = (HttpWebRequest) WebRequest.Create("http://localhost:1477/api/empires/" + Amplitude.Mercury.Interop.Snapshots.GameSnapshot.PresentationData.EmpireInfo[wantedEmpire].EmpireIndex.ToString() + "/brain/analysis-data/territories");
    ...
    {
      GUILayout.Space(12f);
      ref LocalEmpireInfo local = ref Amplitude.Mercury.Interop.Snapshots.GameSnapshot.PresentationData.LocalEmpireInfo;
      if ((double) Time.realtimeSinceStartup > (double) this.nextRefresh)
      {
        HttpWebRequest httpWebRequest = (HttpWebRequest) WebRequest.Create("http://localhost:1477/api/empires/" + local.EmpireIndex.ToString() + "/brain/analysis-data/military-missions");
      ...
      GUILayout.Label(string.Format("{0} military missions in desire set for empire {1}", (object) num, (object) local.EmpireIndex));
      int territoryIndex = (int) Amplitude.Mercury.Interop.AI.Snapshots.World.TileInfo[DebugControl.CursorPosition.ToTileIndex()].TerritoryIndex;
      GUILayout.Label(string.Format("Position {0} | Territory {1}", (object) DebugControl.CursorPosition, (object) territoryIndex));
      for (int index = 0; index < num; ++index)
      {
        ...
        {
          GUILayout.Label(string.Format("- Task {0}: {1} - Motivation: {2}", (object) jtoken[(object) "Id"], (object) (MilitaryObjective) jtoken.Value<int>((object) "Objective"), (object) jtoken[(object) "Motivation"].Value<float>((object) "Value")));
          GUILayout.Label(string.Format("  Army type: {0} - Status: {1} - Reinforcement: {2}", (object) jtoken[(object) "WantedArmyType"], (object) jtoken[(object) "Status"], (object) jtoken[(object) "ReinforcementIndex"]));
        }
      }
    }
    ...
    private void OnDrawArmyDebugger()
    {
      ...
      {
        int empireIndex = (int) Amplitude.Mercury.Interop.Snapshots.ArmyCursorSnapshot.PresentationData.EmpireIndex;
        if ((SimulationEntityGUID) DebugControl.ArmyGuid != Amplitude.Mercury.Interop.Snapshots.ArmyCursorSnapshot.PresentationData.EntityGUID || (double) Time.realtimeSinceStartup > (double) this.nextRefresh)
        {
          DebugControl.ArmyGuid = (ulong) Amplitude.Mercury.Interop.Snapshots.ArmyCursorSnapshot.PresentationData.EntityGUID;
          HttpWebRequest httpWebRequest = (HttpWebRequest) WebRequest.Create("http://localhost:1477/api/empires/" + empireIndex.ToString() + "/brain/army-actuator/" + Amplitude.Mercury.Interop.Snapshots.ArmyCursorSnapshot.PresentationData.EntityGUID.ToString());
          httpWebRequest.BeginGetResponse(new AsyncCallback(this.OnArmyRequestEnd), (object) httpWebRequest);
          if ((SimulationEntityGUID) DebugControl.ArmyGuid != Amplitude.Mercury.Interop.Snapshots.ArmyCursorSnapshot.PresentationData.EntityGUID)
            DebugControl.ArmyRequestResult = (JObject) null;
          this.nextRefresh = Time.realtimeSinceStartup + 1f;
        }
        if (GUILayout.Button("Army " + DebugControl.ArmyGuid.ToString() + " of empire " + empireIndex.ToString()))
          UnityEngine.Application.OpenURL(string.Format("http://localhost:1477/empires/{0}/execution/army-actuators?armyGuid={1}", (object) empireIndex, (object) DebugControl.ArmyGuid));
        ...
        try
        {
          ...
          GUILayout.Label("Behavior tree: " + armyRequestResult["Data"][(object) 0][(object) "BehaviorTreeName"]?.ToString());
          ...
          GUILayout.Label("Mission type: " + jtoken[(object) "MissionInfo"][(object) "MissionType"]?.ToString());
          GUILayout.Label("Mission flags: " + jtoken[(object) "MissionInfo"][(object) "MissionFlags"]?.ToString());
        }
        catch (Exception ex)
        {
        }
    ...
    private void OnDrawTerrainDebugger()
    {
      if (!DebugControl.CursorPosition.IsWorldPositionValid())
        return;
      ref Tile local = ref Amplitude.Mercury.Interop.AI.Snapshots.World.Tiles[DebugControl.CursorPosition.ToTileIndex()];
      GUILayout.Label(string.Format("Position {0} | {1}", (object) DebugControl.CursorPosition, (object) local.MovementType));
      ...
      GUILayout.Label(string.Format("Visible {0}", (object) Amplitude.Mercury.Presentation.Presentation.PresentationVisibilityController.IsTileVisible(DebugControl.CursorPosition.ToTileIndex())));
      GUILayout.Label(string.Format("Explored {0}", (object) Amplitude.Mercury.Presentation.Presentation.PresentationVisibilityController.IsTileExplored(DebugControl.CursorPosition.ToTileIndex())));
    ...
    private void OnDrawAdjacencyDebugger()
    {
      ...
      Amplitude.Mercury.Interop.AI.Entities.Territory territory = Amplitude.Mercury.Interop.AI.Snapshots.World.Territories[(int) local2.TerritoryIndex];
      GUILayout.Label(string.Format("Position {0} | {1}", (object) DebugControl.CursorPosition, (object) local1.MovementType));
      ...
      for (int index = 0; index < territory.Adjacencies.Length; ++index)
        GUILayout.Label(string.Format("Adjacency with {0}: {1}", (object) territory.Adjacencies[index].TerritoryIndex, (object) territory.Adjacencies[index].Transition));
    }

    private void OnDrawCityDebugger()
    {
      if (!(Amplitude.Mercury.Presentation.Presentation.PresentationCursorController.CurrentCursor is SettlementCursor))
        return;
      GUILayout.Label("Settlement of empire " + Amplitude.Mercury.Interop.Snapshots.SettlementCursorSnapshot.PresentationData.EmpireIndex.ToString());
      if (GUILayout.Button("Ask Advice"))
        SandboxManager.PostOrder((Order) new OrderAskAdviseConstructible()
        {
          SettlementGUID = Amplitude.Mercury.Interop.Snapshots.SettlementCursorSnapshot.PresentationData.SettlementGUID
        });
      GUILayout.BeginHorizontal();
      if (GUILayout.Button("<"))
      {
      ...
      if (settlementRequestResult != null && settlementRequestResult["Data"].Any<JToken>())
      {
        ...
        try
        {
          ...
          for (int index = 0; index < source.Count<JToken>(); ++index)
            str1 = str1 + "[" + jtoken2[(object) index]?.ToString() + "/" + source[(object) index]?.ToString() + "]";
          GUILayout.Label("FimsNeed:" + str1);
          WorldPosition highlightedPosition = Amplitude.Mercury.Presentation.Presentation.PresentationCursorController.CurrentHighlightedPosition;
          for (int index1 = 0; index1 < jtoken1[(object) "WantedDistricts"].Count<JToken>(); ++index1)
          {
            if (jtoken1[(object) "WantedDistricts"][(object) index1].Value<int>((object) "TileIndex") == highlightedPosition.ToTileIndex())
            {
              string[] strArray = jtoken1[(object) "WantedDistricts"][(object) index1][(object) "Score"].ToString().Split('\n');
              ...
              GUILayout.Label(string.Format("{0}: {1}", (object) jtoken1[(object) "WantedDistricts"][(object) index1][(object) "DistrictConstructionName"], (object) str2));
            }
          }
        }
        catch (Exception ex)
      ...
      if (!((SimulationEntityGUID) DebugControl.SettlementGuid != Amplitude.Mercury.Interop.Snapshots.SettlementCursorSnapshot.PresentationData.SettlementGUID) && (double) Time.realtimeSinceStartup <= (double) this.nextRefresh)
        return;
      DebugControl.SettlementGuid = (ulong) Amplitude.Mercury.Interop.Snapshots.SettlementCursorSnapshot.PresentationData.SettlementGUID;
      string requestUriString = "http://localhost:1477/api/empires/" + Amplitude.Mercury.Interop.Snapshots.SettlementCursorSnapshot.PresentationData.EmpireIndex.ToString() + "/brain/analysis-data/entities/" + Amplitude.Mercury.Interop.Snapshots.SettlementCursorSnapshot.PresentationData.SettlementGUID.ToString();
      ...
      if ((SimulationEntityGUID) DebugControl.SettlementGuid != Amplitude.Mercury.Interop.Snapshots.SettlementCursorSnapshot.PresentationData.SettlementGUID)
    ...
    private void OnDrawBattleMapDebugger()
    {
      if (!DebugControl.CursorPosition.IsWorldPositionValid())
        return;
      int battle = Amplitude.Mercury.Interop.AI.Snapshots.Battle.BattleMap[DebugControl.CursorPosition.ToTileIndex()];
      if (battle < 0)
        return;
      GUILayout.Label(string.Format("Battle {0} (index: {1})", (object) Amplitude.Mercury.Interop.AI.Snapshots.Battle.Battles[battle].SimulationEntityGUID, (object) battle));
    }
    ...
    private void OnDrawVisibilityDebugger()
    {
      ...
      if ((double) Time.realtimeSinceStartup <= (double) this.nextRefresh)
        return;
      HttpWebRequest httpWebRequest = (HttpWebRequest) WebRequest.Create("http://localhost:1477/api/empires/" + local.EmpireIndex.ToString() + "/brain/visibility-dates");
      ...
      try
      {
        using (HttpWebResponse response = (HttpWebResponse) ((WebRequest) asynchronousResult.AsyncState).EndGetResponse(asynchronousResult))
        {
          using (Stream responseStream = response.GetResponseStream())
          {
            using (StreamReader streamReader = new StreamReader(responseStream))
            {
              DebugControl.VisibilityRequestResult = JArray.FromObject((object) JObject.Parse(streamReader.ReadToEnd())["Data"]);
              DebugControl.UpdatePresentationDebugger = true;
            }
          }
        }
