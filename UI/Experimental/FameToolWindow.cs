...
namespace DevTools.Humankind.GUITools.UI
{
    public class FameToolWindow : FloatingToolWindow
    {
        ...
        {
            using (new GUILayout.VerticalScope((GUIStyle) "Widget.ClientArea",
                    ...
                    if (this.RuntimeService.Runtime != null && this.RuntimeService.Runtime.HasBeenLoaded)
                    {
                        if (this.RuntimeService.Runtime.FiniteStateMachine.CurrentState != null)
                        {
                                ...
                                typeof(RuntimeState_InGame))
                            {
                                ...
                                {
                                    this.deedDefinitionDatabase = this.deedDefinitionDatabase ??
                                                                  Databases.GetDatabase<DeedDefinition>();
                                    this.eraStarDefinitionDatabase = this.eraStarDefinitionDatabase ??
                                                                     Databases
                                                                         .GetDatabase<EraStarDefinition>();
                                    this.competitiveDeedDefinitionDatabase =
                                        this.competitiveDeedDefinitionDatabase ??
                                        Databases.GetDatabase<CompetitiveDeedDefinition>();
                                    this.DrawFame();
                                    this.DrawEraStars();
        ...
        private void DrawFame()
        {
            ...
            using (new GUILayout.HorizontalScope(Array.Empty<GUILayoutOption>()))
            {
                ...
                GUILayout.Label("======================= Fame =======================",
                    (GUIStyle) "PopupWindow.MonospacedLabel");
                GUILayout.FlexibleSpace();
            }
        ...
        {
            int empireIndex = (int) Snapshots.GameSnapshot.PresentationData.LocalEmpireInfo.EmpireIndex;
            FixedPoint eraStarsCount =
                Snapshots.GameSnapshot.PresentationData.EmpireInfo[empireIndex].EraStarsCount;
            FixedPoint starsRequirement = Snapshots.GameSnapshot.PresentationData.EmpireInfo[empireIndex]
                .EraStarsRequirement;
            using (new GUILayout.HorizontalScope(Array.Empty<GUILayoutOption>()))
            {
                GUILayout.FlexibleSpace();
                GUILayout.Label(
                    string.Format("==================== Era Stars ({0}/{1}) ====================",
                        (object) eraStarsCount.Format(), (object) starsRequirement.Format()),
                    (GUIStyle) "PopupWindow.MonospacedLabel");
                GUILayout.FlexibleSpace();
            }
            ...
            using (GUILayout.ScrollViewScope scrollViewScope =
                   new GUILayout.ScrollViewScope(this.eraStarsPosition, ScrollViewStyle,
                       GUILayout.Height(256f)))
            {
                ...
                for (int index1 = 0; index1 < length; ++index1)
                {
                    ...
                    if (eraStarInfo2.PoolAllocationIndex >= 0)
                    {
                        ...
                        if (!this.eraStarDefinitionDatabase.TryGetValue(eraStarInfo2.EraStarDefinitionName,
                                out eraStarDefinition))
                            return;
                        using (new GUILayout.HorizontalScope(Array.Empty<GUILayoutOption>()))
                        {
                            this.stringBuilder.Length = 0;
                            this.stringBuilder.AppendLine(eraStarDefinition.Name.ToString());
                            this.stringBuilder.AppendFormat("Based on Deed '{0}'",
                                (object) eraStarDefinition.DeedDefinitionReference.ElementName);
                            GUILayout.Label(this.stringBuilder.ToString(), GUILayout.Width(384f));
                            this.stringBuilder.Length = 0;
                            this.stringBuilder.AppendFormat("Score = {0} - Reward = {1}",
                                (object) eraStarInfo2.Score, (object) eraStarInfo2.CurrentReward);
                            int num = eraStarDefinition.Levels != null ? eraStarDefinition.Levels.Length : 0;
                            for (int index2 = 0; index2 < num; ++index2)
                            {
                                this.stringBuilder.AppendLine();
                                this.stringBuilder.AppendFormat("  - [{2}] Level {0}: {1}",
                                    (object) (index2 + 1), (object) eraStarInfo2.Thresholds[index2].Format(),
                                    index2 < eraStarInfo2.Level ? (object) "X" : (object) " ");
                            }
                            ...
                            if (eraStarInfo2.Level < num)
                            {
                                if (GUILayout.Button("Turbo!"))
                                    SandboxManager.PostOrder((Order) new OrderForceGainEraStarScore()
                                    {
                                        ...
                                        Gain = (int) (eraStarInfo2.Thresholds[eraStarInfo2.Level] -
                                                      eraStarInfo2.Score)
                                    });
                            }
                        }
            ...
            EraStarCatchupInfo eraStarCatchupInfo = Snapshots.GameSnapshot.PresentationData
                .EmpireInfo[empireIndex].EraStarCatchupInfo;
            this.stringBuilder.Length = 0;
            this.stringBuilder.AppendFormat("Catchup: {0} (+{1} per Turn) / {2} - Level = {3}",
                (object) eraStarCatchupInfo.Score.Format(),
                (object) eraStarCatchupInfo.PointsPerTurn.Format(),
                (object) eraStarCatchupInfo.Threshold.Format(), (object) eraStarCatchupInfo.Level);
            this.stringBuilder.AppendLine();
        ...
        private void DrawCompetitiveDeeds()
        {
            ...
            {
                GUILayout.FlexibleSpace();
                GUILayout.Label("==================== Competitive Deeds ====================",
                    (GUIStyle) "PopupWindow.MonospacedLabel");
                GUILayout.FlexibleSpace();
            }

            using (GUILayout.ScrollViewScope scrollViewScope =
                   new GUILayout.ScrollViewScope(this.competitiveDeedPosition, ScrollViewStyle,
                       GUILayout.Height(256f)))
            {
                int length = Snapshots.FameSnapshot.PresentationData.CompetitiveDeedInfo.Length;
                for (int index1 = 0; index1 < length; ++index1)
                {
                    ref CompetitiveDeedInfo local =
                        ref Snapshots.FameSnapshot.PresentationData.CompetitiveDeedInfo.Data[index1];
                    using (new GUILayout.HorizontalScope(Array.Empty<GUILayoutOption>()))
                    {
                        this.stringBuilder.Length = 0;
                        this.stringBuilder.AppendLine(local.CompetitiveDeedDefinitionName.ToString());
                        this.stringBuilder.AppendFormat("Based on Deed '{0}'",
                            (object) local.DeedDefinitionName);
                        if (!StaticString.IsNullOrEmpty(local.ElementName))
                        {
                            this.stringBuilder.AppendLine();
                            this.stringBuilder.AppendFormat("  - Name = {0}",
                                (object) local.ElementName.ToString());
                        }
                        ...
                        if (local.ElementIndex >= 0)
                        {
                            ...
                            if (local.Type == CompetitiveDeedInfo.DeedType.PerNaturalWonder)
                                this.stringBuilder.AppendFormat("  - {0}",
                                    (object) Snapshots.GameSnapshot.PresentationData
                                        .NaturalWonderInfo[local.ElementIndex].NaturalWonderDefinitionName);
                            else
                                this.stringBuilder.AppendFormat("  - Index = {0}",
                                    (object) local.ElementIndex);
                        }

                        GUILayout.Label(this.stringBuilder.ToString());
                        GUILayout.FlexibleSpace();
                        CompetitiveDeedDefinition competitiveDeedDefinition =
                            this.competitiveDeedDefinitionDatabase.GetValue(local
                                .CompetitiveDeedDefinitionName);
                        string str =
                            !StaticString.IsNullOrEmpty(competitiveDeedDefinition.FameRewardReference
                                .DefinitionReference.ElementName)
                                ? competitiveDeedDefinition.FameRewardReference.DefinitionReference
                                    .ElementName.ToString()
                                : "Custom";
                        this.stringBuilder.Length = 0;
                        this.stringBuilder.AppendFormat("Reward: {0} ({1})", (object) local.FameReward,
                            (object) str);
                        this.stringBuilder.AppendLine();
                        ...
                        else
                        {
                            ...
                            for (int index2 = 1; index2 < winningMajorEmpires; ++index2)
                                this.stringBuilder.AppendFormat(", {0}",
                                    (object) local.WinningMajorEmpireIndices[index2]);
                        }
