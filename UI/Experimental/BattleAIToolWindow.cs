    ...
    public class BattleAIToolWindow : FloatingToolWindow
    {
        ...
        private void DrawBattleMenu()
        {
            if (Amplitude.Mercury.Presentation.Presentation.PresentationCursorController.CurrentCursor is
                BattleUnitCursor currentCursor)
            ...
            DebugControl.AutoSyncBattleDebug = GUILayout.Toggle(DebugControl.AutoSyncBattleDebug, "Auto sync",
                (GUIStyle) "PopupWindow.Button");
            DebugControl.ShowBestBattleDebug = GUILayout.Toggle(DebugControl.ShowBestBattleDebug, "Show Best",
                (GUIStyle) "PopupWindow.Button");
            DebugControl.StepByStepBattles = GUILayout.Toggle(DebugControl.StepByStepBattles, "Step-by-step",
                (GUIStyle) "PopupWindow.Button");
            ...
            else
            {
                ...
                for (int index = 0; index < brainCount; ++index)
                    this.battleLabels[index] = string.Format("{0}",
                        (object) BattleControllerDebug.Brains[index].BattleGUID);
                if (this.currentBattleIndex >= brainCount)
                    this.currentBattleIndex = 0;
                GUILayout.BeginHorizontal();
                GUILayout.Label("Select a battle:");
                GUILayout.Space(2f);
                int num1 = GUILayout.Toolbar(this.currentBattleIndex, this.battleLabels,
                    (GUIStyle) "PopupWindow.ToolbarButton", GUILayout.ExpandWidth(false));
                ...
                GUILayout.Label(string.Format("Battle {0}: {1}", (object) brain.BattleGUID,
                    (object) brain.BattleState));
                GUILayout.Label(string.Format("Brain state: {0} (Empire {1})", (object) brain.BrainState,
                    (object) brain.CurrentEmpireIndex));
                if ((Object) brain.CurrentBattleConfiguration != (Object) null)
                    GUILayout.Label(string.Format("Current battle configuration: {0}",
                        (object) brain.CurrentBattleConfiguration.Name));
                else
                    GUILayout.Label("no battle configuration");
                GUILayout.BeginHorizontal();
                if (GUILayout.Button("Open brain inspector", (GUIStyle) "PopupWindow.Button"))
                    UnityEngine.Application.OpenURL(string.Format("http://localhost:1477/global/battle/{0}",
                        (object) brain.BattleGUID));
                if (GUILayout.Button("Do next step", (GUIStyle) "PopupWindow.Button"))
                    this.httpClient.GetAsync(string.Format(
                        "http://localhost:1477/api/ai-controller/battle/{0}/step",
                        (object) brain.BattleGUID));
                GUILayout.EndHorizontal();
                ...
                else
                {
                    ...
                    GUILayout.Space(2f);
                    int num3 = GUILayout.Toolbar(this.currentUnitIndex, this.unitLabels,
                        (GUIStyle) "PopupWindow.ToolbarButton", GUILayout.ExpandWidth(false));
                    ...
                    GUILayout.Label(string.Format("Action [{0}/{1}] (sorted)",
                        (object) (this.currentActionIndex + 1), (object) num2));
                    if (GUILayout.Button(">", (GUIStyle) "PopupWindow.Button"))
                    {
                        DebugControl.UpdatePresentationDebugger = true;
                        DebugControl.ShowBestBattleDebug = false;
                    ...
                    ActionDebugInfo action2 =
                        brain.Actions[(this.currentActionIndex + 1) % brain.ActionCount];
                    if (this.currentUnitIndex != 0)
                    {
                        int num4 = 0;
                        for (int index = 0; index < brain.ActionCount; ++index)
                    ...
                    switch (action1.Type)
                    {
                        ...
                        case ActionType.Attack:
                            ...
                            GUI.color = Color.red;
                            GUILayout.Label(string.Format("Attack{0} at {1}", (object) action1.TargetGUID,
                                (object) action1.TargetPosition));
                            break;
                        ...
                        case ActionType.MoveAndAttack:
                            GUILayout.Label(string.Format("Unit{0}: Move and attack",
                                (object) action1.ChosenUnitGUID));
                            GUI.color = new Color(0.4f, 1f, 0.43f);
                            GUILayout.Label(string.Format("From{0}", (object) action1.ChosenUnitPosition));
                            GUI.color = new Color(0.12f, 0.56f, 1f);
                            GUILayout.Label(string.Format("To{0}", (object) action1.Destination));
                    ...
                    if ((Object) brain.CurrentBattleConfiguration == (Object) null)
                        return;
                    BattleConfiguration.HeuristicLevel[] heuristicLevelArray =
                        brain.BattleState == BattleState.Deployment
                            ? brain.CurrentBattleConfiguration.DeploymentHeuristicLevels
                            : brain.CurrentBattleConfiguration.CombatHeuristicLevels;
                    BattleConfiguration.ActionComparer actionComparer =
                        brain.BattleState == BattleState.Deployment
                            ? brain.CurrentBattleConfiguration.DeploymentActionComparer
                            : brain.CurrentBattleConfiguration.CombatActionComparer;
                    string text = "->";
                    ...
                    for (int index1 = 0; index1 < heuristicLevelArray.Length; ++index1)
                    {
                        ...
                        {
                            BattleConfiguration.HeuristicConfiguration heuristic =
                                heuristicLevel.Heuristics[index2];
                            float num7 = action1.ScoreByHeuristic[(int) heuristic.ConfigurationHeuristic]
                                .Value;
                            float num8 = action2.ScoreByHeuristic[(int) heuristic.ConfigurationHeuristic]
                                .Value;
                            num5 += num7;
                            num6 += num8;
                            GUILayout.BeginHorizontal();
                            ...
                            GUI.color = (double) num7 - (double) num8 > 0.0001
                                ? Color.yellow
                                : ((double) num7 - (double) num8 < -0.0001 ? Color.blue : Color.white);
                            GUILayout.Label((double) num7 > (double) num8
                                ? " > "
                        ...
                        GUI.color = (double) num9 - (double) num10 > (double) actionComparer.Step
                            ? Color.red
                            : ((double) num9 > (double) num10 ? Color.yellow : Color.white);
                        GUILayout.Label((double) num9 - (double) num10 > (double) actionComparer.Step
                            ? " >> "
