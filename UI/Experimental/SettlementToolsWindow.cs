...
namespace DevTools.Humankind.GUITools.UI
{
    public class SettlementToolsWindow : FloatingToolWindow
    {
        ...
        private static StaticString campRelocationConstructibleName =
            new StaticString("ConstructibleAction_CampRelocationCenter");
        ...
        private void Draw()
        {
            ...
            else if (Snapshots.GameSnapshot == null || Snapshots.SettlementCursorSnapshot == null ||
                     !Snapshots.GameSnapshot.Enabled)
                GUILayout.Label("Waiting for the snapshots...");
            else if (!Snapshots.SettlementCursorSnapshot.Enabled ||
                     !Snapshots.SettlementCursorSnapshot.PresentationData.SettlementGUID.IsValid)
            {
            ...
            else
            {
                ...
                SettlementCursorSnapshot.Data presentationData2 =
                    Snapshots.SettlementCursorSnapshot.PresentationData;
                ...
                if (presentationData2.SettlementStatus == SettlementStatuses.Camp &&
                    presentationData2.EmpireIndex == (int) presentationData1.LocalEmpireInfo.EmpireIndex)
                {
                    GUI.enabled = true;
                    if (GUILayout.Button("CampRelocationCursor", (GUIStyle) "PopupWindow.ToolbarButton",
                            GUILayout.Width(150f)))
                        Amplitude.Mercury.Presentation.Presentation.PresentationCursorController
                            .ChangeToCampRelocationCursor(presentationData2.SettlementGUID,
                                SettlementToolsWindow.campRelocationConstructibleName);
                    ...
                    if (local.FailureFlags == Amplitude.Mercury.Simulation.FailureFlags.None)
                        return;
                    GUILayout.Label(string.Format("Evolve failure flags : {0}.",
                        (object) local.FailureFlags));
                }
                else
                {
                    ...
                    if (presentationData2.CurrentConstructionInfos.Length != 0)
                    {
                        if (presentationData2.CurrentConstructionInfos[0].CanBeBoughtOutWithPopulation)
                        {
                            GUILayout.Label(string.Format(
                                "Population buyout\n Convert '{0}' population into '{1}' industry.",
                                (object) presentationData2.CurrentConstructionInfos[0].PopulationBuyoutCost,
                                (object) presentationData2.CurrentConstructionInfos[0]
                                    .PopulationBuyoutIndustryProduced));
                            if (presentationData2.CurrentConstructionInfos[0].CanAffordPopulationBuyout)
                            {
                                if (GUILayout.Button("Buyout with population"))
                                    SandboxManager.PostOrder(
                                        (Order) new OrderBuyoutConstructionWithPopulationAt()
                                        {
                                            SettlementGUID = presentationData2.SettlementGUID,
                                            ConstructionIndex = 0
                                        });
                    ...
                    if ((local2.FailureFlags & Amplitude.Mercury.Simulation.FailureFlags.ActionLocked) ==
                        Amplitude.Mercury.Simulation.FailureFlags.None)
                    {
                        ...
                        if (local2.IsToggledOn && remainingCooldown < cooldownDuration)
                            GUILayout.Label(string.Format("Money mode cooldown : {0} turns.",
                                (object) (cooldownDuration - remainingCooldown)));
                    ...
                    ref EmpireInfo local3 =
                        ref Snapshots.GameSnapshot.PresentationData.EmpireInfo[(int) empireIndex];
                    GUILayout.Space(10f);
                    GUILayout.Label("Expansionist affinity");
                    if ((local3.ExoticAbilityFlags &
                         EmpireExoticAbilityFlags.CanBuyNeighbourTerritoryWithMoney) ==
                        EmpireExoticAbilityFlags.None)
                    {
                    ...
                    if (GUILayout.Button("Bribe neighbour settlement with money"))
                        Amplitude.Mercury.Presentation.Presentation.PresentationCursorController
                            .ChangeToRallyTerritoryCursor(presentationData2.SettlementGUID);
                    GUI.enabled = true;
                    ...
                    if (local4.FailureFlags != Amplitude.Mercury.Simulation.FailureFlags.None)
                        GUILayout.Label(string.Format("Inquisition failure flags : {0}.",
                            (object) local4.FailureFlags));
                    ...
                    if (local5.FailureFlags != Amplitude.Mercury.Simulation.FailureFlags.None)
                        GUILayout.Label(string.Format("Procession failure flags : {0}.",
                            (object) local5.FailureFlags));
                    ...
                    if (local6.AffinityActionFailureFlags != AffinityActionFailureFlags.None)
                        GUILayout.Label(string.Format("Raise reserviste failure flags : {0}.",
                            (object) local6.AffinityActionFailureFlags));
                    GUI.enabled = presentationData2.SettlementActionInfos[19].FailureFlags ==
                                  Amplitude.Mercury.Simulation.FailureFlags.None;
                    if (GUILayout.Button("Liberate settlement"))
                        SandboxManager.PostOrder((Order) new OrderLiberateSettlement()
                        {
                    ...
                    if (presentationData2.EmpireIndex >= presentationData1.NumberOfMajorEmpires)
                    {
                        if (!Snapshots.MinorPatronageSnapshot.Enabled)
                            Snapshots.MinorPatronageSnapshot.Start(presentationData2.EmpireIndex,
                                MinorPatronageSnapshot.ActivationFlags.DebugWindow);
                        MinorPatronageSnapshot.Data presentationData3 =
                            Snapshots.MinorPatronageSnapshot.PresentationData;
                        ...
                        for (int index1 = length - 1; index1 >= 0; --index1)
                        {
                            ...
                            ref PatronageRelationWithMajor local8 =
                                ref presentationData3.RelationWithMajors[index2];
                            GUILayout.Label(string.Format(" - {0} - Empire {1} with a score of {2}, net {3}",
                                (object) (length - index1), (object) index2, (object) local8.PatronageStock,
                                (object) local8.PatronageNet));
                        }
                        ...
                        Snapshots.MinorPatronageSnapshot.Stop(MinorPatronageSnapshot.ActivationFlags
                            .DebugWindow);
