        ...
        protected void OnDrawWindowContent()
        {
          ...
          GUILayout.Height(500f)
                }))
                {
                    ...
                    {
                        string str = !StaticString.IsNullOrEmpty(empireInfo.EmpireStabilityDefinitionName) ? empireInfo.EmpireStabilityDefinitionName.ToString() : "no city";
                        GUILayout.Label(string.Format("Empire stability : {0} ({1})", (object)empireInfo.EmpireStability, (object)str));
                    ...
                    using (new GUILayout.HorizontalScope(Array.Empty<GUILayoutOption>()))
                    {
                        GUILayout.Label(string.Format("Number of votable civics : {0}", (object)Snapshots.CivicsScreenSnapshot.PresentationData.NumberOfVotableCivicsPerMajorEmpire[(int)this.currentEmpireIndex]));
                        GUILayout.FlexibleSpace();
                    ...
                    for (int index1 = 0; index1 < length1; ++index1)
                    {
                        ...
                        using (new GUILayout.HorizontalScope(Array.Empty<GUILayoutOption>()))
                        {
                            ...
                            GUILayout.Label(string.Format("{0} ({1}) | {2}", (object)civicDefinition.Name, (object)title, (object)civicStatuses));
                            GUILayout.FlexibleSpace();
                            GUI.enabled = num == 0;
                            if (GUILayout.Button("Unlock", (GUIStyle)"PopupWindow.ToolbarButton", GUILayout.Width(60f)))
                                SandboxManager.PostOrder((EditorOrder)new EditorOrderUnlockCivic()
                                {
                                    EmpireIndex = (int)Snapshots.GameSnapshot.PresentationData.LocalEmpireInfo.EmpireIndex,
                            ...
                            if (GUILayout.Button("Cancel", (GUIStyle)"PopupWindow.ToolbarButton", GUILayout.Width(60f)))
                        ...
                        for (int index2 = 0; index2 < length2; ++index2)
                        {
                            using (new GUILayout.HorizontalScope(Array.Empty<GUILayoutOption>()))
                            {
                                ...
                                if (GUILayout.Button("Activate", (GUIStyle)"PopupWindow.ToolbarButton", GUILayout.Width(60f)))
                                ...
                                GUI.enabled = !flag;
                                if (GUILayout.Button("Force", (GUIStyle)"PopupWindow.ToolbarButton", GUILayout.Width(60f)))
                    ...
                    using (new GUILayout.HorizontalScope(Array.Empty<GUILayoutOption>()))
                    {
                        if ((Snapshots.GameSnapshot.PresentationData.EmpireInfo[(int)this.currentEmpireIndex].MiscFlags & EmpireMiscFlags.HasActivatedFirstCivics) > EmpireMiscFlags.None)
                        {
                            ...
                            if (revolutionTimer >= FixedPoint.MaxValue)
                            {
                                ...
                                if (GUILayout.Button("+5 Revolution", (GUIStyle)"PopupWindow.ToolbarButton", GUILayout.Width(120f)))
                                ...
                                if (!GUILayout.Button("Start revolution", (GUIStyle)"PopupWindow.ToolbarButton", GUILayout.Width(120f)))
                                    return;
                            ...
                            else
                            {
                                ...
                                if (!GUILayout.Button("Stop revolution", (GUIStyle)"PopupWindow.ToolbarButton", GUILayout.Width(120f)))
                                    return;
                                SandboxManager.PostOrder((Order)new OrderChangeRevolutionState()
                                {
