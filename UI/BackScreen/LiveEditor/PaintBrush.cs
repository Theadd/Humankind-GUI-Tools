...
namespace DevTools.Humankind.GUITools.UI
{
    public partial class PaintBrush : IWorldTile
    {
        ...
        private Settlement SimulationEntitySettlement { get; set; }
                ...
                if (EmpireIndex != -1)
                {
                    ...
                    else
                    {
                            ...
                            if ((HexTile | HexTileType.Luxury) == HexTile)
                                if ((HexTile | HexTileType.District) == HexTile)
                                    if ((HexTile | HexTileType.Manufactory) != HexTile)
                                        return Create(() => CreateLuxuryManufactoryAt(TileIndex, ResourceNum),
                                            CreateLuxuryManufactoryAction);
                                    else
                                        return false;
                                else
                                    return Create(() => CreateLuxuryResourceExtractorAt(TileIndex),
                                        CreateLuxuryExtractorAction);

                            if ((HexTile | HexTileType.Strategic) == HexTile)
                                if ((HexTile | HexTileType.District) != HexTile)
                                    return Create(
                                        () => CreateStrategicResourceExtractorAt(TileIndex, ResourceNum),
                                        CreateStrategicExtractorAction);
                                else
                                    return false;
                        }
                        ...
                        if ((brushType | LiveBrushType.District) == brushType)
                            return Create(() => CreateDistrictAt(TileIndex, brush.Name),
            ...
            if (HexTile != HexTileType.None)
            {
                ...
                else
                {
                    if ((HexTile | HexTileType.AdministrativeCenter) == HexTile)
                        return Destroy(
                            () => DetachTerritoryFromCity(
                                SimulationEntitySettlement.WorldPosition.ToTileIndex(), TileIndex),
                            DetachTerritoryAction);
        ...
        public void UpdateTile()
        {
            ...
            ref TileInfo tileInfo = ref Amplitude.Mercury.Interop.Snapshots.GameSnapshot.PresentationData
                .LocalEmpireInfo.TileInfo.Data[TileIndex];
            EmpireIndex = -1;
            ...
            var terrainType = (int) tileInfo.TerrainType < data.TerrainTypeDefinitions.Length
                ? data.TerrainTypeDefinitions[(int) tileInfo.TerrainType]
                : null;
                    ...
                    out ISerializable sArmy))
            {
                ...
                if (district != null)
                {
                    ...
                    else if (district.DistrictDefinition.Name ==
            ...
            PlacementCursorController.Run(TileIndex);
        }
    }
}
